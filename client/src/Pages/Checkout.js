...
import { geturl } from '../Session/Session';
const Checkout = (props) => {
    ...
    useEffect(()=>{
        ...
        setitems(props.location.state);
            if(props.location.state.items.length === 1){
                showLoading();
                axios.get(geturl() + '/product/'+props.location.state.items[0].productid)
                .then((res)=>{
                    const data = {
                        id:res.data.id,
                        img:res.data.img,
    ...
    async function pay_api() {
        ...
        if(Products.length === 1){
            ...
            //console.log(order);
            showLoading();
            const url = geturl() + '/order/add';
            axios.post(url,order)
                .then((res)=>{
                    if(checkoutproduct.items[0].id){
                        axios.delete(geturl() +'/cart/'+checkoutproduct.items[0].id);
                    }

                    axios.get(geturl() + '/product/'+Products[0].id)
                        .then((res)=>{
                            ...
                            const url = geturl() + '/product/update/'+ Products[0].id;
